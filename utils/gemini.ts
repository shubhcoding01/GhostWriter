...
// ⚠️ IMPORTANT: Verify your API Key is pasted here correctly
const API_KEY = "Your Api Key";

export async function fixTextWithAI(roughNotes: string, style: string = "Professional", language: string = "English") {
  // 1. Safety Check
  if (!API_KEY || API_KEY.includes("YOUR_GEMINI")) {
    console.error("Error: API Key is missing or invalid.");
    return null; 
  }
  ...
  try {
    ...
    // 🛠️ FIX: Switched from "gemini-1.5-flash" to "gemini-pro" (More stable)
    ...
    const prompt = `
      ...
      Output: ONLY the rewritten text. Do not add conversational filler.
    `;

    // 4. Timeout Logic (10 seconds)
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error("Timeout: Internet too slow")), 10000)
    );
